    ...
    for period, group in grouped:
        ...
        for cat in categories:
            ...
            if np.isnan(data["Dp17OError"]).any() == False:
                ax.errorbar(group["dateTimeMeasured"], group["Dp17O"],
...
dfInfo = pd.read_csv(os.path.join(sys.path[0], "UG sample info.csv"))
dfMerged = pd.merge(df_avg, dfInfo, on="SampleName")
dfMerged.to_csv(os.path.join(sys.path[0], "UG Table S4.csv"), index=False)
...
plt.savefig(os.path.join(sys.path[0], "UG Figure S4.png"))
plt.close("all")
